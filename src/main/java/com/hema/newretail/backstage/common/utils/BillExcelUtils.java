...
public class BillExcelUtils {
    ...
    //根据指定的excel模板导出数据
    public static void exportOrderExcel(String orderSrcFileName, List<OrdersDataBo> data, OutputStream fos, String time) throws Exception {
        ...
        for (int i=0;i<data.size();i++){
            ...
            newRow.getCell(0).setCellValue(i+1);
            newRow.getCell(1).setCellValue(order.getId());
            newRow.getCell(2).setCellValue(TimeUtil.getStringByDate(order.getProductionTime()));
            newRow.getCell(3).setCellValue(order.getMenuName());
            /**
             * 订单状态已付款 1未制作，2，不能做，3，超时，6已处在制作队列中，
             * 7确认完毕放弃制作，8退款中，9主动退款成功，10补偿金退款，11退款失败，129制作中，
             * 135未领取，143已领取
             * */
            StringBuffer status = new StringBuffer();
            if(CommonConstants.STRINGPOSITIVEINTEGER_ONE.equals(order.getOrderStatus())){
                status.append("未制作");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_TWO.equals(order.getOrderStatus())){
                status.append("不能做");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_THREE.equals(order.getOrderStatus())){
                status.append("超时");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_SIX.equals(order.getOrderStatus())){
                status.append("已处在制作队列中");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_SEVEN.equals(order.getOrderStatus())){
                status.append("确认完毕放弃制作");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_EIGHT.equals(order.getOrderStatus())){
                status.append("退款中");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_NINE.equals(order.getOrderStatus())){
                status.append("主动退款成功");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_TEN.equals(order.getOrderStatus())){
                status.append("补偿金退款");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_ELEVEN.equals(order.getOrderStatus())){
                status.append("退款失败");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_129.equals(order.getOrderStatus())){
                status.append("制作中");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_135.equals(order.getOrderStatus())){
                status.append("未领取");
            }
            if(CommonConstants.STRINGPOSITIVEINTEGER_143.equals(order.getOrderStatus())){
                status.append("已领取");
            }
            newRow.getCell(4).setCellValue(status.toString());
            newRow.getCell(5).setCellValue(order.getDeviceNumber());
            newRow.getCell(6).setCellValue(order.getBillId());
            newRow.getCell(7).setCellValue(order.getRemark());
        }
